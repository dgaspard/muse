...
- Use the API workflow example to upload documents and derive stories.
- Inspect API logs via `docker compose logs -f api` to confirm validation and story generation.
- Smoke tests in `scripts/smoke_test.sh` verify core service health.
